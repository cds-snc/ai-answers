...
**"Still working" / in-progress indicators are not `StatusMessage`'s job — use `src/components/admin/Loading.js`.** It used to have a `loading` sub-type; that was pulled out into `LoadingStatus` (inline spinner + text, in normal page flow) and `LoadingOverlay` (full-page/section backdrop with a centered spinner+text box) — two components, not a flag, because they render genuinely different shapes and merging them back behind a prop is exactly what caused a real bug: `loading` and `variant` were two independent visual modes whose block-content requirement had to be reconciled by hand in three separate places, and one was missed, so the spinner ended up nested inside an invalid parent tag. Determinate progress (a known total, e.g. "chunk 3 of 10") isn't `StatusMessage`'s job either, and doesn't belong in `Loading.js` as a third mode — see `ExperimentalAnalysisPage.js`'s `renderProgressCards` for the established pattern (a real `role="progressbar"` + a plain `role="status"` text line). If a component's props fork behavior in more than one place at once (a tag choice here, a CSS class there, content there) for what's really a different *kind* of thing, split it into a separate component instead of adding another flag — don't wait for a second bug to prove the point.

```jsx
import { LoadingStatus, LoadingOverlay } from '../components/admin/Loading.js';

{loading && <LoadingStatus message={t('some.page.loading')} />}
