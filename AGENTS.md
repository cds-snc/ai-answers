...
Known gap (tracked as a TODO in the component itself): it only standardizes the ARIA behaviour so far — there's no built-in `variant` prop yet, so callers still wire up box styling themselves via `className`/`tag` rather than StatusMessage picking it automatically from `isError`/a variant. Three GC DS-token-based box classes already exist in `admin.css` for this and should be reused, not recreated: `dashboard-error` (red-100/500/700, failures), `dashboard-warning-box` (yellow-100/500/700, cautions like unsaved changes), `dashboard-info-box` (blue-100/500/700, neutral confirmations) — each pairs with a `GcdsIcon` (`warning-triangle` for error/warning, `info-circle` for info) passed as `children` rather than `message`, since the box only shows once there's actual content. `dashboard-error` additionally has a `dashboard-error--inline` modifier (`width: fit-content`) for a message that shouldn't stretch to its container's full width. These will keep evolving — expect them to fold into StatusMessage's own `variant` prop eventually per the TODO, and expect the class names themselves (`dashboard-error`, etc.) to be generalized/renamed once more use cases beyond dashboards define what the shared shape actually needs to be. Don't add a fourth one-off box style or copy an existing page's ad-hoc hex colours; extend or reuse these three, and expect their names to change out from under you.
