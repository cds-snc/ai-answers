...
const { mockAssignQuestion } = vi.hoisted(() => ({ mockAssignQuestion: vi.fn() }));
vi.mock('../../../services/DashboardService.js', () => ({ default: { assignQuestion: mockAssignQuestion } }));

describe('useChatAssignBar', () => {
  beforeEach(() => {
    mockGetAssignable.mockReset();
    mockAssignQuestion.mockReset();
  });
  ...
  it('setting the same mode again is a no-op (details fires toggle on mount with open=true)', async () => {
    ...
    act(() => { result.current.toggleQuestionChecked('q-1', true); });
    act(() => { result.current.setAssignMode(true); });
  ...
  it('ignores a second submitAssign while one is in flight', async () => {
    let resolveFirst;
    mockAssignQuestion.mockImplementationOnce(() => new Promise((r) => { resolveFirst = r; }));
    const { result } = renderHook(() => useChatAssignBar());
    act(() => {
      result.current.toggleQuestionChecked('q-1', true);
    ...
    expect(mockAssignQuestion).toHaveBeenCalledTimes(1);
  ...
  it('resets checked chats and selected assignee when mode is toggled off', async () => {
    ...
    expect(result.current.isQuestionChecked('q-1')).toBe(false);
  });

  it('submitAssign calls assignChat once per checked chat, with the selected assignee and note', async () => {
    mockAssignQuestion.mockResolvedValue({});
    const { result } = renderHook(() => useChatAssignBar());

    act(() => {
      result.current.toggleQuestionChecked('q-1', true);
      result.current.toggleQuestionChecked('q-2', true);
      result.current.setSelectedAssigneeId('u1');
    ...
    expect(mockAssignQuestion).toHaveBeenCalledTimes(2);
    expect(mockAssignQuestion).toHaveBeenCalledWith({ interactionId: 'q-1', assignedTo: 'u1', notes: 'please review' });
    expect(mockAssignQuestion).toHaveBeenCalledWith({ interactionId: 'q-2', assignedTo: 'u1', notes: 'please review' });
  ...
  it('surfaces a no_expert validation error instead of calling assignChat, when no assignee is picked', async () => {
    ...
    expect(mockAssignQuestion).not.toHaveBeenCalled();
  ...
  it('on a partial failure, reports the failure count, unticks the ones that worked, and reloads', async () => {
    ...
    expect(result.current.isQuestionChecked('q-1')).toBe(false); // succeeded
    expect(result.current.isQuestionChecked('q-2')).toBe(true); // failed, stays for retry
    expect(result.current.selectedCount).toBe(1);
    expect(onDone).toHaveBeenCalled();
  });

  it('forgetQuestion drops a chat from the selection without clearing the outcome message', async () => {
    mockAssignQuestion.mockResolvedValue({});
    const { result } = renderHook(() => useChatAssignBar());
    act(() => { result.current.toggleQuestionChecked('q-1', true); result.current.toggleQuestionChecked('q-2', true); result.current.setSelectedAssigneeId('u1'); });
    await act(async () => { await result.current.submitAssign(); });
    act(() => { result.current.toggleQuestionChecked('q-3', true); });
    act(() => { result.current.forgetQuestion('q-3'); });
    expect(result.current.selectedCount).toBe(0);
    expect(result.current.isQuestionChecked('q-3')).toBe(false);
  });
  ...
  it('reports one shared failure reason as its code (409 already assigned), mixed reasons as none', async () => {
    ...
    mockAssignQuestion.mockRejectedValue(conflict);
    const { result } = renderHook(() => useChatAssignBar());
    act(() => { result.current.toggleQuestionChecked('q-1', true); result.current.toggleQuestionChecked('q-2', true); result.current.setSelectedAssigneeId('u1'); });
    await act(async () => { await result.current.submitAssign(); });
    ...
    mockAssignQuestion.mockReset();
    mockAssignQuestion.mockRejectedValueOnce(conflict).mockRejectedValueOnce(Object.assign(new Error('nope'), { status: 403 }));
    act(() => { result.current.toggleQuestionChecked('q-3', true); result.current.toggleQuestionChecked('q-4', true); });
    await act(async () => { await result.current.submitAssign(); });
  ...
  it('does not reload when every assign failed', async () => {
    mockAssignQuestion.mockRejectedValue(new Error('nope'));
    const { result } = renderHook(() => useChatAssignBar());
    act(() => {
      result.current.toggleQuestionChecked('q-1', true);
      result.current.setSelectedAssigneeId('u1');
  ...
  it('clears a stale assignStatus when the assignee dropdown, a checkbox, or the note text changes', async () => {
    ...
    act(() => { result.current.toggleQuestionChecked('q-1', true); result.current.setSelectedAssigneeId('u1'); });
    await act(async () => { await result.current.submitAssign(); });
    expect(result.current.assignStatus).not.toBeNull();
    act(() => { result.current.toggleQuestionChecked('q-2', true); });
    expect(result.current.assignStatus).toBeNull();
