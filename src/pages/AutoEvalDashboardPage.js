...
const AutoEvalDashboardPage = ({ lang = 'en' }) => {
  ...
  const columns = useMemo(() => ([
    ...
    { title: t('admin.chatDashboard.columns.aiEval', 'AI Eval'), data: 'aiEval', render: v => { if (!v) return ''; const label = t(`admin.chatDashboard.labels.evaluation.${v}`, v); return `<span class="label ${escapeHtmlAttribute(v)}">${escapeHtmlAttribute(label)}</span>`; }, searchable: true, orderable: true },
    { title: t('admin.chatDashboard.columns.partnerEval', 'Partner Eval'), data: 'partnerEval', render: v => { if (!v) return ''; const label = t(`admin.chatDashboard.labels.evaluation.${v}`, v); return `<span class="label ${escapeHtmlAttribute(v)}">${escapeHtmlAttribute(label)}</span>`; }, searchable: true, orderable: true },
