  ...
  get privatePatterns() {
    return [
      ...
      },
      {
        ...
        description:
          'Alphanumeric sequences of 5+ chars that contain both letters and numbers (catches various ID numbers, passport numbers, account codes, etc., but excludes pure text or numbers)',
      },
      {
      ...
      },
      {
        ...
        pattern:
          /\b(Mr\.?|Mrs\.?|Ms\.?|Miss|Dr\.?|Prof\.?|Sir|Madam|Lady|Monsieur|Madame|Mademoiselle|Docteur|Professeur)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/g,
        description: 'Names with prefixes',
      },
      {
        // Names in "My name is..." format
        pattern:
          /\b(?:my name is|je m'appelle|je me nomme|my name's|i am|i'm|je suis)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/gi,
        description: 'Names in introduction phrases',
      },
      {
        // Capitalized names (2-3 words)
        pattern:
          /\b([A-Z][a-z]{1,20}(?:\s+[A-Z][a-z]{1,20}){1,2})\b(?!\s+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Court|Ct|Lane|Ln|Way|Parkway|Pkwy|Square|Sq|Terrace|Ter|Place|Pl|Circle|Cir|Loop))\b/g,
        description: 'Capitalized names (2-3 words, not followed by street type)',
      },
      {
        // Names in greeting patterns
        pattern:
          /\b(?:Dear|Hello|Hi|Bonjour|Cher|Chère|Salut)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/gi,
        description: 'Names in greeting patterns',
      },
      {
        // Names in signature patterns
        pattern:
          /\b(?:Sincerely|Regards|Best|Cheers|Cordialement|Sincèrement|Amicalement)\s*,\s*\n*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/gi,
