...
const ChatAppContainer = ({ lang = 'en', chatId }) => {
  ...
  const updateStatusWithTimer = useCallback(
    (status) => {
              ...
              text: (
                <div
                  dangerouslySetInnerHTML={{
                    __html: error.redactedText.includes('XXX')
  ...
  const formatAIResponse = useCallback(
    (aiService, message) => {
      ...
      const finalConfidenceRating = message.interaction.confidenceRating
        ? message.interaction.confidenceRating
        : '0.1';
      ...
      return (
        <div className="ai-message-content">
          ...
          {message.interaction.answer.answerType === 'normal' &&
            (message.interaction.answer.citationHead || displayUrl) && (
