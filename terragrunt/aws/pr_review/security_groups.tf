...
resource "aws_security_group_rule" "ai_answers_lambda_egress_redis" {
  count = var.env == "staging" ? 1 : 0

  description              = "Security group rule for AI Answers egress to Redis"
  type                     = "egress"
  from_port                = 6379
  to_port                  = 6379
  protocol                 = "tcp"
...
resource "aws_security_group_rule" "redis_ai_answers_lambda_ingress" {
  count = var.env == "staging" ? 1 : 0

  description              = "Security group rule for AI Answers Redis ingress"
  type                     = "ingress"
  from_port                = 6379
  to_port                  = 6379
  protocol                 = "tcp"
