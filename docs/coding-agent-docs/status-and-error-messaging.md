...
- **"Loading…" is skippable.** `LoadingOverlay` and `StatusMessage loading` announce with `skippable: true` — a short grace, then dropped if a result *announcement* lands first. Unmounting the overlay drops nothing, so whatever renders one must follow with a completion announcement ("Results loaded." below; `ServerDataTable.js` announces its own) or a fast load reads "Loading…" after the content has already appeared. Use `skippable` for in-progress states, never outcomes.
...
- **`LoadingOverlay`** (`src/components/admin/LoadingOverlay.js`) — full-page backdrop for when nothing else on the page is actionable until the operation finishes: the filter-driven dashboards, and `ScenarioOverridesPage.js`'s data load and Save/Revert. If a page's controls are all disabled for a stretch with an inline `StatusMessage loading` beside them, it's probably this. `scoped` covers one region instead (a table sharing a page with other live content — `ServerDataTable.js`; see [dashboards.md](dashboards.md) for the CSS pairing it needs). Same `useAnnounceOnChange` as `StatusMessage`; separate file because it's narrow, not structurally different.
