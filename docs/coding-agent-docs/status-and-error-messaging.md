...
Treat this as a genuinely different product from the admin/partner tooling, not the same app with inconsistent patterns. Admin is dozens of pages, dashboards, and forms, each with its own save/delete/import/export/loading outcomes — that scale is what the `StatusMessage`/`FeedbackInlineError`/`liveAnnouncer.js` machinery exists for. The live chat has a handful of states total (loading, one error, one disclaimer) on one screen. Its own small, self-contained announcement and focus system, tuned and validated with screen-reader users, is a deliberate fit for that much smaller surface — not a shortcut or historical debt. Stays separate on purpose, easy to maintain as is: don't migrate it onto `StatusMessage`/`announce()`, and don't copy its patterns out to admin pages.
