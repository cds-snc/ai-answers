...
- **Don't set DataTables' own `processing: true` on a table already covered by `LoadingOverlay`.** DataTables only ever toggles that node's `display`, never re-inserts it or changes its text, so most screen readers never announce it regardless of its `role="status"` — and visually it renders underneath `LoadingOverlay`'s full-page backdrop anyway. `ServerDataTable.js` tracks its own `loading` state and renders a scoped `LoadingOverlay` instead. `ExperimentalServerDataTable.js` still sets `processing: true` — it's a near-identical fork of `ServerDataTable.js` from before this fix existed, not a case that needs the flag; it just hasn't adopted the scoped-`LoadingOverlay` pattern yet. See the TODO in `ServerDataTable.js`'s top comment about eventually replacing it outright.
