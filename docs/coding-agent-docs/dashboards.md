...
- **Loading states — by trigger, not by page:**
  ...
  - One bounded region instead of the whole page (a table sharing a page with other live content) → `<LoadingOverlay scoped message={...} />` — same component, `position: absolute` instead of `fixed`; the caller's wrapper needs its own `position: relative` to anchor it.
...
- **Don't set DataTables' own `processing: true` on a table already covered by `LoadingOverlay`.** DataTables only ever toggles that node's `display`, never re-inserts it or changes its text, so most screen readers never announce it regardless of its `role="status"` — and visually it renders underneath `LoadingOverlay`'s full-page backdrop anyway. `ServerDataTable.js`/`ExperimentalServerDataTable.js` are the exception: nothing else on their pages covers the table while it loads, so `processing: true` is the only loading signal those tables have — keep it there.
