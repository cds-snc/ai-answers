...
`src/components/admin/StatusMessage.js` is the single component every save/delete/import/export/test-run/upload outcome, autosave failure, or general-purpose loading state should render through (see AGENTS.md's "Announcing status, errors, and async outcomes" for the usage-level API — `message`/`isError`/`loading`/`variant`/`nonce`/`announce`/`id`). It now has **five built-in states**, each pulling its box styling from GC DS-token classes in `admin.css` — reuse one of these, don't hand-roll a new colour/icon combination for a sixth:
