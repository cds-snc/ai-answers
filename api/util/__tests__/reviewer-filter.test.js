...
describe('resolveReviewerMatch', () => {
  ...
  it('unknown institution: matches nothing, without querying', async () => {
    const result = await resolveReviewerMatch({ institution: 'NOT-A-DEPT' });
    expect(mockUserFind).not.toHaveBeenCalled();
  ...
  it('repeated query param (array): matches nothing, without querying', async () => {
    const result = await resolveReviewerMatch({ group: ['A', 'B'] });
    expect(mockUserFind).not.toHaveBeenCalled();
    expect(result).toEqual({ userIds: [], feedbackIds: [] });
  });

  it('overlong reviewerEmail: matches nothing, without querying', async () => {
    const result = await resolveReviewerMatch({ reviewerEmail: 'a'.repeat(129) });
    expect(mockUserFind).not.toHaveBeenCalled();
    expect(result).toEqual({ userIds: [], feedbackIds: [] });
  });
