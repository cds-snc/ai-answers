 ...
 * - runs: completed-run comparisons executed against the dataset, oldest first
...
async function handler(req, res) {
    try {
        ...
        const runs = await ExperimentalBatch.find({
            ...
            .select('name runLabel appVersion status createdAt config.analyzerIds config.analyzerId config.baselineRunId config.candidateRunId config.trials summary')
            .sort({ createdAt: -1 })
            .limit(MAX_RUNS)
            .lean();
