...
async function handler(req, res) {
    try {
        ...
        if (req.method === 'GET') {
            return res.json(await ExperimentalDatasetService.previewGoldenAnswerDataset(startDate, endDate));
        }
        if (req.method === 'POST') {
            return res.status(201).json(await ExperimentalDatasetService.createGoldenAnswerDataset({
                startDate,
                endDate,
                name: request.name,
