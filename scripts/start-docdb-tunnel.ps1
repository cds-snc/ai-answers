<#
.SYNOPSIS
    Starts an AWS Session Manager tunnel from an ECS Exec-enabled task to DocumentDB.
...
.EXAMPLE
    .\scripts\start-docdb-tunnel.ps1 `
...
function Get-DocumentDbHost {
    if ($DocumentDbHost) {
        return $DocumentDbHost
    }

    if ($env:DOCDB_HOST) {
        return $env:DOCDB_HOST
    }
    ...
    try {
        $docdbUri = Invoke-AwsText -Arguments $ssmArgs
        if (-not $docdbUri) {
            throw "Empty docdb_uri parameter"
        }
        return [uri]$docdbUri | Select-Object -ExpandProperty Host
    } catch {
        throw "Could not infer the DocumentDB host from SSM parameter 'docdb_uri'. Pass -DocumentDbHost or set DOCDB_HOST explicitly."
    }
}
...
$DocumentDbHost = Get-DocumentDbHost
...
$parameterFile = Join-Path ([System.IO.Path]::GetTempPath()) ("docdb-tunnel-{0}.json" -f ([guid]::NewGuid().ToString("N")))
...
Write-Host "Opening tunnel: localhost:${LocalPort} -> ${DocumentDbHost}:${RemotePort}"
