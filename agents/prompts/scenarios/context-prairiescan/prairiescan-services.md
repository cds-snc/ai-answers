# PrairiesCan services

Curated list of Prairies Economic Development Canada services used to keep
service tagging consistent across questions. One service per row.

PrairiesCan is part of the ISED portfolio but has its own scenario folder, so its
services are curated here rather than in `context-ised-isde/ised-isde-services.md`.
The rows below were seeded from the programs named in `prairiescan-scenarios.js`.
