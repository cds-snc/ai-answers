...
export async function buildAnswerSystemPrompt(language = 'en', options = {}) {
  try {
    ...
    const fullPrompt = `\n      ${ROLE}\n\n      ## Current date\n      <current-date>${currentDate}</current-date>
      ...
      <training-cutoff>June 2024</training-cutoff>

      Use the current date above to determine temporal context. Content published after the training cutoff (June 2024) may be unfamiliar and should be downloaded for verification. When answering questions about future events or recent announcements, do not cite outdated sources (e.g., 2022 news content for 2025+ events). When answering questions about the past, use the past tense if the date is before the current date.
