...
async function contextSearch(query) {
  console.log(
    `Starting search with query: ${query} at endpoint: ${process.env.CANADA_CA_SEARCH_URI}`
  );
...
const contextSearchTool = tool(
  ...
  },
  {
    name: 'canadaCASearch',
    description:
      "Perform a search using Coveo or Google. Provide the 'query' as the search term and optionally 'searchService' ('google' or 'canadaca')",
    schema: {
      type: 'object',
      properties: {
        query: {
