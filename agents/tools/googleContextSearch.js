...
function extractSearchResults(results, numResults = 3) {
  ...
  const extractedResults = topResults
    .map(
      (result) => `Title: ${result.linkText}\nLink: ${result.link}\nSummary: ${result.summary}\n`
    )
    .join('\n');
...
const contextSearchTool = tool(
  ...
  },
  {
    name: 'contextSearch',
    description:
      "Perform a search on Google Custom Search. Provide 'query' as the search term. Example input: { query: 'What is SCIS? Canada' }",
    schema: {
      type: 'object',
      properties: {
        query: {
