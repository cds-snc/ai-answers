...
graph.addNode('init', async (state) => {
    ...
    // Use the virtual provider for GPT-5 Mini
    const selectedAI = 'azure-gpt5-mini';
    ...
    await ServerLoggingService.info('Starting GPT5MiniDefaultGraph', state.chatId, {
        lang: state.lang,
        referringUrl: state.referringUrl,
        selectedAI: selectedAI,
    });

    const out = { startTime, status: WorkflowStatus.MODERATING_QUESTION, selectedAI };
...
export const gpt5MiniDefaultGraphApp = graph.compile();

export async function runGPT5MiniDefaultGraph(input) {
    return gpt5MiniDefaultGraphApp.invoke(input);
}

export default gpt5MiniDefaultGraphApp;
