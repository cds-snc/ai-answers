...
class RedactionService {
  ...
  redactText(text = '', lang = 'en') {
    ...
    // Scanner / exploit probes — automated bot traffic hitting the chat endpoint
    // (e.g. CVE-2017-9841, the PHPUnit eval-stdin.php RCE). These are structural
    // signatures (request lines, exploit paths, traversal), not vocabulary words,
    // so they can't live in the manipulation word-list. Tagged 'manipulation' so
    // processRedaction hard-blocks them just like the word-list matches.
    const maliciousProbePatterns = [
      // Verbose probe description: "GET request to /path", "POST request /path"
      /\b(?:GET|POST|PUT|DELETE|HEAD|PATCH|OPTIONS|TRACE|CONNECT)\b\s+request\s+(?:to\s+)?\/\S+/gi,
      // Raw HTTP request line: an UPPERCASE method directly followed by a path
      // (case-sensitive on purpose so lowercase "can you get /help" doesn't trip)
      /\b(?:GET|POST|PUT|DELETE|HEAD|PATCH|OPTIONS|TRACE|CONNECT)\s+\/\S+/g,
      // Known scanner directories / sensitive dotfiles appearing in a path
      /\/(?:vendor|wp-admin|wp-content|wp-includes|wp-login|phpmyadmin|cgi-bin|\.git|\.env|\.aws|\.ssh)\b\S*/gi,
      // Paths ending in an exploit-prone file type
      /\/\S*\.(?:php|phtml|phar|asp|aspx|jsp|jspx|cgi|sh|bash|bak|env|sql|ini|conf)\b/gi,
      // Directory traversal
      /(?:\.\.[\/\\]){2,}/g,
    ];
    for (const pattern of maliciousProbePatterns) {
