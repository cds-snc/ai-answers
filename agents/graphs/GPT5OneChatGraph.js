...
graph.addNode('init', async (state) => {
    ...
    // Use the GPT-5.1 Chat provider
    const selectedAI = 'openai-gpt51-chat';
    ...
    await ServerLoggingService.info('Starting GPT5OneChatGraph', state.chatId, {
        lang: state.lang,
        referringUrl: state.referringUrl,
        selectedAI: selectedAI,
    });
...
export const gpt5OneChatGraphApp = graph.compile();

export async function runGPT5OneChatGraph(input) {
    return gpt5OneChatGraphApp.invoke(input);
}

export default gpt5OneChatGraphApp;
